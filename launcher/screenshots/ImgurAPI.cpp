...
{
    auto getPayload = [shot] -> std::expected<QHttpMultiPart*, QString> {
    ...
    auto parseFunc = [shot](const QByteArray& response) -> Net::RPC::Sink<QString>::ParseResult {
        ...
        shot->m_imgurId = data.value("id").toString();
        shot->m_url = data.value("link").toString();
        shot->m_imgurDeleteHash = data.value("deletehash").toString();
        return shot->m_url;
    };

    auto dl = Net::Request::makeCustomRequest(
        { .method = Net::HttpMethod::Post, .url = QUrl(BuildConfig.IMGUR_BASE_URL + "image"), .data = getPayload });
...
{
    // this payload needs to be generated after the screenshots are uploaded
    auto getPayload = [screenshots]() {
        QStringList hashes;
        for (const auto& shot : screenshots) {
            hashes.append(shot->m_imgurDeleteHash);
        }
        return "deletehashes=" + hashes.join("&deletehashes=").toUtf8() + "&title=Minecraft%20Screenshots&privacy=hidden";
    };
    ...
    auto dl = Net::Request::makeCustomRequest(
        { .method = Net::HttpMethod::Post, .url = QUrl(BuildConfig.IMGUR_BASE_URL + "album"), .data = getPayload });
    auto sink = std::make_unique<Net::RPC::Sink<AlbumResult>>(std::move(parseFunc));
    auto* result = sink->result();
    dl->setSink(std::move(sink));
    dl->addHeaderProxy(std::make_unique<Net::RawHeaderProxy>(QList<Net::HeaderPair>{
